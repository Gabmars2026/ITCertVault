...
function valid(n,cfg){const t=clean(n.title).toLowerCase().replace(/^\(pr\)\s*/,'').replace(/\s+-\s+[^-]{2,80}$/,'').trim(),s=cfg.name.toLowerCase().trim();if(!t||t.length<8)return false;if(t===s)return false;if(t.replace(/[^a-z0-9]+/g,'')===s.replace(/[^a-z0-9]+/g,''))return false;return true}
...
function dedupePreferDirect(direct,google){const out=[];for(const n of [...direct,...google]){if(out.some(x=>sameStory(x,n)))continue;out.push(n)}return out}
module.exports=async function(req,res){
  ...
  const jobs=[cfg.feed?getText(cfg.feed):Promise.resolve(''),getText(googleUrl(cfg))];
  ...
  const google=results[1].status==='fulfilled'?parse(results[1].value,cfg.name).filter(recent).filter(n=>valid(n,cfg)):[];
  let items=dedupePreferDirect(direct,google).sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt)).slice(0,50);
  ...
  res.status(200).json({updatedAt:new Date().toISOString(),refreshSeconds:60,maxAgeDays:15,source:key,name:cfg.name,count:items.length,ranking:'newest-first-deduped',news:items});
};
