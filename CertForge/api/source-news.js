const SOURCES={
  ...
  'cybernews':{name:'Cybernews',feed:'',site:'cybernews.com',query:'(security OR cybersecurity OR cybercrime OR ransomware OR malware OR vulnerability OR breach OR hacking OR threat OR critical infrastructure)',refreshSeconds:900},
...
module.exports=async function(req,res){
  ...
  const direct=cfg.feed&&results[0].status==='fulfilled'?parse(results[0].value,cfg.name):[];
  ...
  res.status(200).json({updatedAt:new Date().toISOString(),refreshSeconds:cfg.refreshSeconds||60,maxAgeDays:15,source:key,name:cfg.name,count:items.length,news:items});
};
