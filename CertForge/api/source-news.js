...
module.exports=async function(req,res){
  ...
  const direct=cfg.feed&&results[0].status==='fulfilled'?parse(results[0].value,cfg.name).filter(recent):[];
  const google=results[1].status==='fulfilled'?parse(results[1].value,cfg.name).filter(recent):[];
