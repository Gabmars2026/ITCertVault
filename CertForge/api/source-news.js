...
module.exports=async function(req,res){
  ...
  const all=dedupePreferDirect(direct,google).sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt));
  ...
  res.status(200).json({updatedAt:new Date().toISOString(),refreshSeconds:cfg.refreshSeconds,maxAgeDays:15,source:key,name:cfg.name,count:items.length,totalCount,page,perPage:PAGE_SIZE,totalPages,maxPages:MAX_PAGES,hasPrev:page>1,hasNext:page<totalPages,ranking:'newest-first-deduped',news:items});
};