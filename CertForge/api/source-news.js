const SOURCES={
  ...
  'securityweek':{name:'SecurityWeek',feed:'https://www.securityweek.com/feed/',site:'securityweek.com',query:'(cybersecurity OR vulnerability OR breach OR malware OR CISA)'},
...
module.exports=async function(req,res){
  ...
  let direct=results[0].status==='fulfilled'?parse(results[0].value,cfg.name):[];
  let google=results[1].status==='fulfilled'?parse(results[1].value,cfg.name):[];
  let items=[...direct,...google],seen=new Set();
  items=items.filter(n=>{const k=n.title.toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ').slice(0,12).join(' ');if(seen.has(k))return false;seen.add(k);return true}).sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt)).slice(0,50);
  ...
  res.status(200).json({updatedAt:new Date().toISOString(),source:key,name:cfg.name,count:items.length,news:items});
};