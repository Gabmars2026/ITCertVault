...
function collapse(items){const out=[];for(const n of items){const idx=out.findIndex(x=>Math.abs(new Date(x.publishedAt)-new Date(n.publishedAt))<48*36e5&&similar(x,n));if(idx<0){out.push(n);continue}if(quality(n)>quality(out[idx]))out[idx]=n}return out.sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt))}
...
module.exports=async function(req,res){
  ...
  const news=collapse([...(data.news||[])].filter(keep).sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt))).slice(0,30);
  const severityCounts=news.reduce((a,n)=>(a[n.severity]=(a[n.severity]||0)+1,a),{BREAKING:0,MAJOR:0,DEVELOPING:0});
  res.status(200).json({...data,updatedAt:new Date().toISOString(),refreshSeconds:60,count:news.length,severityCounts,ranking:'newest-first-direct-source-preferred-deduplicated-breaking-tech',news});
};
