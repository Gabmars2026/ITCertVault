...
if (!fs.existsSync(basePath)) throw new Error('base.html was not found in the CertForge project root.');
...
// Keep the original CertForge branding shown in the approved site design.
...
if (markerPos < 0) throw new Error('The CertForge base application marker was not found.');
const scriptPos = html.lastIndexOf('<script', markerPos);
if (scriptPos < 0) throw new Error('The CertForge application script could not be located.');
...
window.addEventListener('error',function(e){
  setTimeout(function(){
    if(document.body && !document.body.innerText.trim()){
      ...
      box.innerHTML='<strong style="color:#f97316;font-size:22px">CertForge startup error</strong><br><br>'+String((e&&e.message)||'Unknown browser error');
...
window.addEventListener('unhandledrejection',function(e){
  setTimeout(function(){
    if(document.body && !document.body.innerText.trim()){
      var box=document.createElement('div');
      box.style.cssText='position:fixed;inset:24px;z-index:2147483647;padding:24px;background:#161b22;color:#ffb4a8;border:1px solid #f97316;border-radius:16px;font:16px/1.5 Arial,sans-serif;overflow:auto';
      box.innerHTML='<strong style="color:#f97316;font-size:22px">CertForge startup error</strong><br><br>'+String((e&&e.reason)||'Unhandled startup rejection');
      document.body.appendChild(box);
    }
  },100);
});
...
console.log(`Built direct CertForge production page (${html.length.toLocaleString()} characters).`);
