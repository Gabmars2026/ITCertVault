...
function titleFor(img){const a=img.closest('article,.breakitem,.pop,.brief-story,.brief-lead');const t=a?.querySelector('h1,h2,h3,strong')?.textContent||img.alt||document.title||'News';return t.trim().slice(0,110)||'News'}
function backup(img,reason='duplicate'){if(!img.isConnected||img.dataset.uniqueBackup==='1')return;const title=titleFor(img),seed=[location.pathname,title,++serial,reason].join('|');img.dataset.uniqueBackup='1';img.dataset.dedupeChecked='1';img.removeAttribute('srcset');img.src=`/api/article-art?seed=${enc(seed)}&title=${enc(title)}`}
