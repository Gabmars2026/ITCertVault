...
function fillToCount(primary,all,count=30){const out=[],seen=new Set();for(const item of [...primary,...all]){const k=item.link||item.id||item.title;if(!k||seen.has(k))continue;seen.add(k);out.push(item);if(out.length>=count)break}return out}
async function news(category='home'){try{let r=await fetch('/api/news?category='+encodeURIComponent(category),{cache:'no-store'});if(!r.ok)throw 0;let j=await r.json();return j.news?.length?j.news:F}catch{return F}}

function upgradeHeader(){
  const top=document.querySelector('.top');if(!top)return;top.classList.add('reference-header');
  const brand=top.querySelector('.brand');if(brand)brand.innerHTML=`<span class="brand-favicon-shell"><img class="brand-favicon" src="/favicon.svg" alt="AI News Now"></span><span class="brand-copy"><strong>AI NEWS NOW</strong><small>Your Daily Source for Everything AI</small></span>`;
  const search=top.querySelector('.search');if(search){const b=search.querySelector('button');if(b)b.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"></circle><path d="m16 16 4 4"></path></svg>`;}
  const actions=top.querySelector('.actions');if(actions)actions.innerHTML=`<button class="reference-action" id="theme" type="button" aria-label="Toggle light and dark theme" title="Toggle theme"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3.4"></circle><path d="M12 2.2v2.1M12 19.7v2.1M4.2 4.2l1.5 1.5M18.3 18.3l1.5 1.5M2.2 12h2.1M19.7 12h2.1M4.2 19.8l1.5-1.5M18.3 5.7l1.5-1.5"></path></svg></button><a class="reference-action bell" href="/breaking/" aria-label="Breaking news" title="Breaking news"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.5 9.8a5.5 5.5 0 0 1 11 0c0 6 2.2 6.1 2.2 6.1H4.3s2.2-.1 2.2-6.1"></path><path d="M9.8 19a2.5 2.5 0 0 0 4.4 0"></path></svg></a>`;
}
...
function home(n){
  ...
  const homeFeed=fillToCount(n,n,30);
  return `<main class="dashboard">
    ...
    <section class="panel home-feed"><div class="pt"><span>MORE AI NEWS</span><a href="/all-news/">View all →</a></div><div class="sectiongrid home-sectiongrid">${homeFeed.map((x,i)=>card(x,i)).join('')}</div></section>
  </main>`
}

function section(n,key){
  let q=new URLSearchParams(location.search).get('q')||'',m=M[key]||M['all-news'],title=m[0],desc=m[1],ks=m[2],items=n;
  if(key==='breaking')items=n.slice(0,30);
  else if(key==='search'){
    title=q?`Search: “${q}”`:'Search AI News';desc=q?`Live results matching ${q}.`:'Use the search box to find companies, models, tools and topics.';items=q?n.filter(x=>text(x).includes(q.toLowerCase())):n;
  }else if(key==='all-news')items=n.slice(0,60);
  else{
    const matches=n.filter(x=>ks.some(k=>text(x).includes(k)));
    items=fillToCount(matches,n,30);
  }
  return `<main class="section"><div class="sectionhero"><div><a class="back" href="/">← AI News Now</a><span class="live">● LIVE FEED</span><h1>${e(title)}</h1><p>${e(desc)}</p></div><div class="stat"><strong>${items.length}</strong><span>stories loaded</span><small>Refreshes automatically</small></div></div>${items.length?`<div class="sectiongrid">${items.map((x,i)=>card(x,i)).join('')}</div>`:`<div class="simple"><div class="box"><h2>No matching live stories right now.</h2><p>The collector will add matching articles automatically as new RSS items arrive.</p><a href="/all-news/">Browse all AI news</a></div></div>`}</main>`
}
...
upgradeHeader();
const searchForm=document.querySelector('.search');if(searchForm)searchForm.onsubmit=x=>{x.preventDefault();let q=x.currentTarget.querySelector('input').value.trim();if(q)location.href='/search/?q='+encodeURIComponent(q)};
const themeBtn=document.getElementById('theme');if(themeBtn)themeBtn.onclick=()=>document.documentElement.classList.toggle('light');
...
(async()=>{let app=document.getElementById('app');if(key==='about'||key==='subscribe'){app.className='';app.innerHTML=special(key);return}let feedKey=key==='search'?'all-news':key;let n=await news(feedKey);app.className='';if(key==='home'){app.innerHTML=home(n);bindHero();startHero()}else app.innerHTML=section(n,key)})();
